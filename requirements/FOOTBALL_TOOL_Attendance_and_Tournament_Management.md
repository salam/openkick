...
- **Homepage:** The homepage at `/` displays the club logo, club name, and club description — all taken from settings. No hardcoded "OpenKick" text appears on the homepage. The server injects these values (and all meta/OG tags, JSON-LD structured data, and Twitter Card tags) into the HTML at request time so that crawlers and social media previews see the correct branding. **The homepage MUST be a Next.js Server Component — never `'use client'`.** See "Server-side rendering of branding" below for the full list of requirements.
...
- **Privacy Policy:** A dedicated `/privacy` page displays a GDPR-compliant privacy policy. Template sections (data collected, purpose/legal basis, retention, rights) are pre-written and localised. The DPO name/email and responsible entity are pulled from settings. A free-text field (`privacy_extra`) allows custom additions. GDPR rights link to the operational endpoints (data export, deletion requests).
- **Server-side rendering of branding (CRITICAL — NO CLIENT-SIDE FALLBACK):** The homepage and all public pages MUST render with the actual club name, club description, club logo, and all meta/OG/Twitter tags **in the initial HTML response** from the server. This is non-negotiable:
  - The `<title>` tag MUST contain `{club_name} — {club_description}`, never a hardcoded fallback like "OpenKick".
  - The favicon `<link>` MUST point to the uploaded club logo.
  - Open Graph tags (`og:title`, `og:description`, `og:image`, `og:url`, `og:type`) MUST be present in the server-rendered `<head>`.
  - Twitter Card tags (`twitter:card`, `twitter:title`, `twitter:description`, `twitter:image`, `twitter:site`) MUST be present in the server-rendered `<head>`.
  - `meta[name="description"]` and `meta[name="keywords"]` MUST be present in the server-rendered `<head>`.
  - JSON-LD structured data (`@type: SportsOrganization` or `Organization`) with club name, logo, description, and contact information MUST be embedded as a `<script type="application/ld+json">` block in the server-rendered `<head>`.
  - The homepage component (`page.tsx`) MUST be a **Server Component** (no `'use client'` directive) that fetches settings on the server before rendering. Client-side interactivity (admin customizer, language toggle) must be isolated in child Client Components.
  - **No `useEffect`-based patching of `document.title` or `document.head` is acceptable as the primary mechanism.** A client-side `DynamicHead` may exist only for live preview during admin editing, never as the production rendering path.
  - **Regression test:** An automated test MUST verify that the raw HTML response from `/` contains the configured club name, not "OpenKick", and includes OG/Twitter/JSON-LD tags with real values.
- **Settings UI:** All branding settings are managed from a "Club Settings" page in the admin dashboard. Changes take effect immediately (no restart required). The settings page is organised into 5 navigable groups: General (Club Profile, Homepage Appearance, Bot Language), Integrations (LLM, WAHA, SMTP, Holidays), Security (Audit, Bot Protection, Security Contact), Content (Feeds, SEO & Social Media, Legal & Privacy), and Team (Users). A sidebar navigation with scroll-spy (Intersection Observer) highlights the active section; on mobile it collapses to a horizontal pill bar. The page header and save button are sticky (top header bar + floating bottom save bar when unsaved changes exist).
